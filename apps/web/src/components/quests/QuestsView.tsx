...
import { getQuests, type Quest } from '@repo/data';
...
function QuestCard({ quest }: { quest: Quest }) {
  ...
  return (
    <div className={`bg-[--card] border rounded-xl p-4 flex items-center gap-4 ${quest.completed ? 'border-green-800/40 opacity-70' : 'border-[--border]'}`}>
...
function QuestList({ quests }: { quests: Quest[] }) {
  ...
  const totalXpEarned = quests.filter((q) => q.completed).reduce((s, q) => s + q.xp, 0);
  ...
  return (
    <div className="flex gap-8 px-6 py-6">
      <FeedWrapper>
        ...
        <section>
          <h2 className="text-xs font-semibold text-[--muted-foreground] uppercase tracking-wider mb-3">Weekly Challenges</h2>
          <div className="flex flex-col gap-2">
...
export function QuestsView({ source }: Props) {
  const { isSignedIn } = useAuth();

  // In mock mode: service provides fake progress — no Convex needed
  const mockQuests = source === 'mock' ? getQuests('mock') : null;

  // In real mode: Convex provides live per-user progress
  const convexQuests = useQuery(
    api.quests.getUserQuests,
    source === 'real' && isSignedIn ? {} : 'skip',
  );

  // Mock mode — synchronous, always ready
  if (source === 'mock' && mockQuests) {
    return <QuestList quests={mockQuests} />;
  }

  // Real mode — show zeroed quests (from service) while Convex loads, then swap
  const quests = convexQuests ?? getQuests('real');
