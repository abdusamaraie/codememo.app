...
import { cheatSheetEntries } from '@repo/mock-data';
import type { CheatSheetEntrySeed } from '@repo/mock-data';
...
export function CheatSheetPanel({ open, onOpenChange, languageSlug, sectionTitle }: Props) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:w-[480px] overflow-y-auto bg-[--background] border-[--border] z-[80]"
      >
        <SheetHeader className="pb-3">
          <SheetTitle className="text-lg">Quick Reference</SheetTitle>
        </SheetHeader>
                  ...
                  <div className="space-y-2">
                    {cat.items.map((entry, i) => (
                      ...
                      >
                        ...
                        <pre className="mt-1.5 text-xs font-mono text-[--foreground] bg-[--background] rounded px-2 py-1.5 overflow-x-auto whitespace-pre-wrap">
                          {entry.syntax}
                        </pre>
                        ...
                        {entry.example && (
                          <pre className="mt-1.5 text-[11px] font-mono text-[--muted-foreground] bg-[--background] rounded px-2 py-1 overflow-x-auto whitespace-pre-wrap">
                            {entry.example}
                          </pre>
                        )}
                      </div>
